...
describe('SkillGenerateDialog', () => {
    ...
    it('skips generation when skip is selected', async () => {
        ...
        render(_jsx(SkillGenerateDialog, { taskDescription: "Review the backlog", actionCount: 4, onClose: onClose }));
        fireEvent.click(screen.getByText('skill.optionSkip').closest('button'));
        fireEvent.click(screen.getByText('common.confirm').closest('button'));
        await waitFor(() => {
    ...
    it('generates once for the current task', async () => {
        ...
        fireEvent.click(screen.getByText('common.confirm').closest('button'));
        await waitFor(() => {
            expect(invoke).toHaveBeenCalledWith('skill:generate', expect.objectContaining({
    ...
    it('persists always-generate preference before generating', async () => {
        const { SkillGenerateDialog } = await import('../SkillGenerateDialog');
        render(_jsx(SkillGenerateDialog, { taskDescription: "Review the backlog", actionCount: 4, onClose: onClose }));
        fireEvent.click(screen.getByText('skill.optionAlways').closest('button'));
        fireEvent.click(screen.getByText('common.confirm').closest('button'));
        await waitFor(() => {
