...
vi.mock('../RelationBadge', () => ({
    default: ({ label, value, onClick }) => onClick ? (_jsx("button", { type: "button", onClick: onClick, children: `${label}:${value}` })) : (_jsx("span", { children: `${label}:${value}` })),
}));
...
describe('ResultPanel', () => {
    ...
    it('renders visual trace summary details from rawOutput', () => {
        render(_jsx(ResultPanel, {}));
        expect(screen.getByText('taskPanels.visualTrace')).toBeInTheDocument();
        expect(screen.getByText('The action and selector hint suggest a visually ambiguous interaction')).toBeInTheDocument();
        ...
        expect(screen.getAllByText((_, element) => element?.textContent?.replace(/\s+/g, ' ').trim() === 'score: 92').length).toBeGreaterThan(0);
