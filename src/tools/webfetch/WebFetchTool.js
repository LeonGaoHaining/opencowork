...
async function fetchWithRetry(url, options, uaRotation, proxyHandler) {
...
export const webfetchTool = tool(async (params) => {
    ...
    try {
        response = await fetchWithRetry(params.url, {
            method: params.method ?? 'GET',
            headers: baseHeaders,
            redirect: params.redirect ?? 'follow',
            signal: AbortSignal.timeout(timeout),
        }, uaRotation, proxyHandler);
    ...
    name: 'webfetch',
    description: 'Fetches content from a specified URL with support for multiple formats (text/markdown/html), UA rotation for anti-blocking, and proxy support.',
    schema: z.object({
