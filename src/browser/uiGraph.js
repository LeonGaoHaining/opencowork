 ...
 */
import { ElementRole, ElementVisibility, DEFAULT_OBSERVER_CONFIG, } from '../types/uiElement';
/**
 * 构建页面UIGraph
 */
...
function buildUIElement(dom, config) {
 ...
 * 生成稳定的CSS路径 - 改进版
 */
function generateStableCSSPath(dom) {
    const parts = [dom.tag];
...
function generateElementId(dom) {
...
function categorizeElements(url, elements) {
    const navigation = [];
    const inputs = [];
    const actions = [];
    const content = [];
