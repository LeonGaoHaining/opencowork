...
export enum RecoveryStrategy {
  ...
  USE_FALLBACK_SELECTOR = 'use_fallback_selector',
...
export class RecoveryEngine {
  ...
  private handleNavigationError(context: RecoveryContext): RecoveryAction {
    return {
  ...
  private handleTimeoutError(context: RecoveryContext): RecoveryAction {
  ...
  private handleClickError(context: RecoveryContext): RecoveryAction {
    const { retryCount } = context;
  ...
  private handleGenericError(context: RecoveryContext): RecoveryAction {
    const { retryCount } = context;
...
${JSON.stringify(
  currentGraph.elements.slice(0, 30).map((e) => ({
    id: e.id,
    role: e.role,
    label: e.label,
    selector: e.selector,
...
Generate a precise CSS selector. Output ONLY the selector string.`;
  ...
  private getActionTargetType(action: AnyAction): string {
    switch (action.type) {
      case 'browser:click':
        return 'button';
      case 'browser:input':
  ...
  private giveUp(reason: string): RecoveryAction {
