...
export class TaskEngine {
  ...
  async checkSkillGenerationAfterTask(
  ...
  ): Promise<SkillCandidateSummary | null> {
    ...
    try {
      ...
      if (skillSettings.autoGenerate) {
        return await this.autoGenerateSkillFromExternal(
          skillActions,
          taskDescription,
          skillSettings.triggerThreshold
        );
      ...
      return {
        status: 'suggested',
        description: `Task completed with ${actionCount} actions and is ready to be saved as a reusable skill candidate.`,
        actionCount,
        threshold: skillSettings.triggerThreshold,
        autoGenerated: false,
        useCases: ['Promote repeatable task to reusable skill', 'Review actions before publishing'],
        inputSpec: 'Task description and the last successful action sequence.',
        outputSpec: 'A draft skill card for review, editing, and later reuse.',
        failureHints: ['Verify the action sequence is stable before publishing', 'Trim unnecessary steps before turning into a skill'],
      };
    } catch (error) {
  ...
  private async autoGenerateSkillFromExternal(
    ...
    if (!this.skillGenerator) return null;
        ...
        if (result.success) {
          ...
          return {
            ...
            description:
              ...
              `Auto-generated from task: ${taskDescription}`,
            actionCount: normalizedActions.length,
            threshold,
            autoGenerated: true,
            source: result.skill?.frontmatter.source,
            inputSpec:
              result.skill?.frontmatter.argumentHint ||
              'Task description and required runtime context.',
            outputSpec:
              result.skill?.description ||
              'Reusable skill draft stored in the skill directory.',
            useCases: result.skill?.frontmatter.tags || ['Repeat this workflow later'],
            failureHints: ['Review the generated skill before broad reuse'],
          };
        }

        return {
          status: 'suggested',
          description: result.error || 'Skill generation was attempted but did not complete successfully.',
          actionCount: normalizedActions.length,
          threshold,
          autoGenerated: true,
          failureHints: ['Open the skills panel to retry with adjusted metadata'],
        };
      }
    } catch (error) {
