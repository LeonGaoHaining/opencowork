...
(function (ReplanTrigger) {
    ReplanTrigger["ACTION_FAILED"] = "action_failed";
    ReplanTrigger["SELECTOR_INVALID"] = "selector_invalid";
    ReplanTrigger["NAVIGATION_ERROR"] = "navigation_error";
    ReplanTrigger["TIMEOUT"] = "timeout";
    ReplanTrigger["USER_REJECTED"] = "user_rejected";
...
export class Replanner {
    ...
    async executeAskUser(question, options) {
        const askUserAction = {
            id: generateId(),
        ...
        try {
            const result = await this.askUserExecutor.execute(askUserAction);
    ...
    async handleSelectorFailure(request) {
    ...
    async generateNewSelector(pageContent, errorMessage) {
        const systemPrompt = `你是一个CSS选择器生成专家。根据页面HTML和错误信息，生成针对目标元素的选择器。

要求：
1. 分析页面结构，找到目标元素
2. 只生成 2-3 个针对当前操作的选择器
3. 优先使用精确选择器：ID、类名、属性
4. 避免使用模糊选择器如 "button"、"input"、"a"
5. 如果是点击操作，优先匹配可见的按钮元素
6. 如果是输入操作，优先匹配输入框元素
...
请生成多个可靠的选择器。`;
    ...
    async handleNavigationError(request) {
    ...
    async handleActionFailed(request) {
        const { error, executionState, remainingPlan } = request;
...
{"shouldRetry": true/false, "strategy": "策略名", "reason": "理由", "newSelector": "新选择器(可选)", "newAction": "新动作描述(可选)"}`;
    ...
    async decideWithRetry(request, retryCount = 0) {
