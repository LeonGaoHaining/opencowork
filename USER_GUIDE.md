...
│   │   ├── executor/      # 执行器
│   │   │   ├── ExecutorRouter.ts
│   │   │   ├── BrowserExecutor.ts    # 浏览器操作（含force:true）
│   │   │   ├── CLIExecutor.ts
│   │   │   ├── AskUserExecutor.ts    # 用户交互执行器
│   │   │   └── ScreencastService.ts  # 实时截图服务
│   │   │
│   │   ├── planner/      # 任务规划
│   │   │   ├── TaskPlanner.ts
│   │   │   ├── PlanExecutor.ts
│   │   │   └── Replanner.ts          # 动态重规划器（LLM驱动）
│   │   │
│   │   └── runtime/       # 运行时引擎
│   │       ├── TaskEngine.ts        # 含重试循环
│   │       └── TakeoverManager.ts
